...
namespace GasHimApi.API.Services
{
    ...
    {
        private readonly IReadRepository<Substance> _substanceRepo;
        private readonly IReadRepository<Process> _processRepo;
        ...
        public ChainsService(IReadRepository<Substance> substanceRepo,
                             IReadRepository<Process> processRepo,
        ...
        public async Task<List<List<string>>> GetChainsByStartOnly(string startSubstance, CancellationToken ct)
        {
            var substances = await _substanceRepo.GetAllAsync(ct);
            var processes = await _processRepo.GetAllAsync(ct);
        ...
        public async Task<List<List<string>>> GetChainsByTargetOnly(string targetSubstance, CancellationToken ct)
        {
            var substances = await _substanceRepo.GetAllAsync(ct);
            var processes = await _processRepo.GetAllAsync(ct);
            var enriched = EnrichProcesses(processes);
        ...
        public async Task<List<List<string>>> GetChainsByStartAndTarget(string start, string target, CancellationToken ct)
        {
            var substances = await _substanceRepo.GetAllAsync(ct);
            var processes = await _processRepo.GetAllAsync(ct);
            var enriched = EnrichProcesses(processes);
        ...
        public async Task<List<List<string>>> GetAllChains(CancellationToken ct)
        {
            var substances = await _substanceRepo.GetAllAsync(ct);
            var processes = await _processRepo.GetAllAsync(ct);
            var enriched = EnrichProcesses(processes);
        ...
        private List<EnrichedProcess> EnrichProcesses(IEnumerable<Process> processes)
        {
            ...
            {
                Name = p.Name!,
                Inputs = ParseSubstances(p.PrimaryFeedstocks!),
                Outputs = ParseSubstances(p.PrimaryProducts!)
            }).ToList();
        }
