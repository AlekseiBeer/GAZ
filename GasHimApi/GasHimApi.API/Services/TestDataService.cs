    ...
    public class TestDataService : ITestDataService
    {
        ...
        public async Task SeedProcessesAsync(int count, CancellationToken ct)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    Name = name,
                    MainInputs = string.Join("; ", mainInputs),
                    AdditionalInputs = string.Join("; ", additionalInputs),
                    MainOutputs = string.Join("; ", mainOutputs),
                    AdditionalOutputs = string.Join("; ", additionalOutputs),
                    YieldPercent = Math.Round(_rnd.NextDouble() * 100, 2)
                });
            }
